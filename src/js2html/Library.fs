...
module Renderer =
    ...
    { new DotLiquid.FileSystems.IFileSystem with
        member this.ReadTemplateFile(context, templateName) =
          ...
          if not (File.Exists(fullPath)) then
            failwithf "File not found: %s" fullPath
  ...
  let propertyName (s : string) = s.Replace("@", "").Replace(":", "")

  // JsonValue to Hash
  let rec private asModel (target : JsonValue) =
    target.Properties()
    |> Seq.map
         (fun (key, value) -> (propertyName key, value |> asModelValue :> obj))
    |> dict
    |> Hash.FromDictionary

  and private asModelValue v =
    match v with
    | JsonValue.String x -> x :> obj
    | JsonValue.Float x -> x :> obj
    | JsonValue.Number x -> x :> obj
    | JsonValue.Null -> null :> obj
    | JsonValue.Boolean x -> x :> obj
    | JsonValue.Array x -> x |> Seq.map asModelValue :> obj
    | JsonValue.Record _ -> asModel v :> obj
  ...
  let parsedTemplates =
    System.Collections.Concurrent.ConcurrentDictionary<_, _>()
  ...
  let private renderAs (templateType : string) =
    ...
    let t =
      parsedTemplates.GetOrAdd
        (templatePath,
         Template.Parse((fromFile templatePath |> Async.RunSynchronously)))
  ...
  let useage = """
js2html [--file FILE | --json "{}"] --templatedir DIRECTORY [--output OUTPUT]
"""
  ...
  let main args =
    ...
    let scalar (arg, def) =
      if args.ContainsKey arg then args.[arg] |> Seq.head
      else def

    let list arg =
      if args.ContainsKey arg then args.[arg]
      else Seq.empty

    let templates = scalar ("templatedir", "./views")
    if not (args.ContainsKey "templatedir") then failwith useage
    ...
    match scalar ("json", ""), list "file" with
    | "", xs when Seq.isEmpty xs -> failwith useage
    | "", xs ->
      AsyncSeq.ofSeq xs
      |> AsyncSeq.map apply
      |> AsyncSeq.iter
    ...
    | json, _ -> generateFromJson template json |> Console.Write
